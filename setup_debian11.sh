...
# ==============================================================================
# 脚本名称: Debian 11 VPS 初始化全能脚本 (适配版)
# 功能: SSH加固 / UFW / Fail2Ban(读auth.log) / BBR / NTP / 日志权限
# ==============================================================================
...
# net-tools 用于 netstat 检查，dnsutils 用于 dig 等
...
# 重启 rsyslog 确保它获取到新文件的句柄
systemctl restart rsyslog
echo -e "${GREEN}日志权限已修正为 640 (root:adm) 并重启日志服务。${NC}"
...
update_sshd_config "UseDNS" "no"                          # 禁用 DNS 反查加快登录
update_sshd_config "PermitRootLogin" "prohibit-password"  # 允许Root密钥登录 (Deb11标准)
...
# 尝试立即步进时间 (如果 chronyc 报错则忽略，Deb11有时需要等待服务完全启动)
...
# 写入 SSH 专用规则 (jail.d)
# 注意：Debian 11 上强制指定 backend = polling 以确保稳定读取 auth.log
...
echo -e "${GREEN}==============================================${NC}"
echo -e "${GREEN}             Debian 11 初始化完成！            ${NC}"
echo -e "${GREEN}==============================================${NC}"
